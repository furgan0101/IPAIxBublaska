...
# LLM boilerplate leaking into a "report" — synthetic disinfo generated at
# scale and pasted unedited (the prompt/refusal text gives it away).
AI_TEXT_MARKERS: tuple[str, ...] = (
    "as an ai",
    "as a language model",
    "i cannot fulfill",
    "i cannot fulfil",
...
# Provenance flags in media URLs/filenames (stand-in for a real C2PA
# content-credentials check on the attached media).
AI_MEDIA_MARKERS: tuple[str, ...] = (
    "ai_generated",
    "ai-generated",
    "deepfake",
    "synthetic",
...
def _media_urls(report: RawReport) -> list[str]:
    """Every media URL attached to a report (legacy field + live media list)."""
    urls: list[str] = [report.media_url] if report.media_url else []
    for item in report.media:
        if item.url:
            urls.append(item.url)
        if item.preview_url:
            urls.append(item.preview_url)
    return urls
...
def _assess_heuristics(report: RawReport) -> Assessment:
    ...
    for marker in AI_TEXT_MARKERS:
        if marker in text:
            return Assessment(False, "Linguistic: AI-Generated Text Leak", 0.05)

    for url in _media_urls(report):
        lowered = url.lower()
        if any(marker in lowered for marker in AI_MEDIA_MARKERS):
            return Assessment(
                False, "Linguistic: AI-Generated Media (C2PA Flagged)", 0.08
            )
...
# --- Incident-level security policy (auto-alerting) ---------------------------------

# High-severity incidents at or above this corroboration confidence skip the
# operator click and go straight to the Leitstelle.
AUTO_DISPATCH_MIN_CONFIDENCE: float = 0.85


def should_auto_dispatch(incident: VerifiedIncident) -> bool:
    """Automatic Alert Dispatch rule: high severity + very high credibility."""
    return (
        not incident.dispatched
        and incident.severity == "high"
        and incident.confidence_score >= AUTO_DISPATCH_MIN_CONFIDENCE
    )


def auto_dispatch(incident: VerifiedIncident) -> VerifiedIncident:
    """Mark an incident as auto-dispatched and announce it on the console.

    The SOP checklist stays open — automatic alerting hands the incident to
    the command staff, it does not work the tasks."""
    now = datetime.now(timezone.utc)
    print(
        f"[AUTO-ALERT] High-severity incident {incident.id} "
        f"({incident.event_type}, {round(incident.confidence_score * 100)}% "
        "confidence) automatically dispatched to LRA Konstanz Command Staff.",
        flush=True,
    )
    return incident.model_copy(update={"dispatched": True, "dispatched_at": now})
